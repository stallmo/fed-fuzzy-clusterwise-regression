...
## How to run
Prerequisites:
- Python 3.9+ and project dependencies installed (see project `pyproject.toml` and `fed-fuzzy-clust-regression/pyproject.toml`)
...
## The Data
The datasets are available from the KEEL repository (https://sci2s.ugr.es/keel/category.php?cat=reg) in the regression section.
For convenience, the datasets are also provided in the `data/` folder.

### Results reported in the paper
The paper reports results on the following datasets to compare with prior work:
- mortgage.dat
- treasury.dat
- wizmir.dat
- delta_elv.dat
...
Results on more datasets from the KEEL repository can easily be generated by following the instructions above.
However, for datasets that do not follow the standard KEEL format, implementing a new dataloader may be required.
